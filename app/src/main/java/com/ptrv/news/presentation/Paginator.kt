...
class Paginator<T>(
    ...
    private val cacheRequestFactory: () -> Single<List<T>>,
    ...
    private fun loadFromCache() {
        disposable?.dispose()
        disposable = cacheRequestFactory.invoke()
            .subscribe(
                { currentState.newData(it) },
                {it.printStackTrace()}
            )
    ...
    private inner class CACHE_PROGRESS : State<T> {

        override fun newData(data: List<T>) {
            if (data.isNotEmpty()) {
                currentState = DATA_FROM_CACHE()
                currentData.clear()
                currentData.addAll(data)
                currentPage = 0
                viewController.showData(true, currentData)
    ...
    private inner class DATA_FROM_CACHE : State<T> {


        override fun refresh() {
            currentState = REFRESH_CACHE()
            viewController.showRefreshProgress(true)
            loadPage(FIRST_PAGE)
        }
    ...
    private open inner class REFRESH : State<T> {
    ...
    private inner class REFRESH_CACHE: REFRESH() {
